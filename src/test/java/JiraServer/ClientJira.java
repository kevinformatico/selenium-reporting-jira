...
public class ClientJira {
	...
	public static String crearEjecucionJIRA(String idCiclo, String idIssue, String idProyecto, String version) {
		...
		try {
			...
			if(response.getStatus() == 200) {
				...
				System.out.println("La instancia de ejecuci�n no se ha creado correctamente");
			}
	...
	public static void updateExecution(String idEjecucion, String estado) {
		...
		try {
			...
			if(response.getStatus() != 200) {
				System.out.println("No se ha logrado actualizar el estado de la ejecuci�n: " + response.getEntityTag());
			}
		
		}catch(Exception e) {
			System.out.println("Ha ocurrido un error al actualizar el estado de ejecuci�n");
		}
	...
	public static String retornaVersionIdJIRA(String idProyecto) {
		...
		try {
			...
			if(response.getStatus() == 200) {
				JSONObject json = new JSONObject(strJSON);
				if(!strJSON.contains("Versi�n")) {
					versionID = "-1";
				}else {
					versionID = json.getJSONArray("unreleasedVersions").getJSONObject(1).get("value").toString();
				}
			}else {
				System.out.println("No se ha encontrado la versi�n del proyecto id: " + idProyecto);
			}
			
		}catch(Exception e) {
			System.out.println("Ha ocurrido un error al retornar la versi�n del proyecto: " + e.getMessage());
		}
...
//			pathFile + " \" " + pathJira + "/rest/zapi/latest/attachment?entityId=" + idEntity + "&entityType=" + entityType); //stepresult - execution
	...
	public static void elminaEjecucionJIRA(String idEjecucion) {
		...
		try {
			...
			if(response.getStatus() != 200) {
				System.out.println("No existe la ejecuci�n con id: " + idEjecucion);
			}
			
		}catch(Exception e) {
			System.out.println("Ha ocurrido un error al eliminar la ejecuci�n: " + e.getMessage());
		}
	...
	public static ArrayList<String> retornaListaResultadosPasosJIRA(String idEjecucion){
		...
		try {
			...
			if(response.getStatus() == 200) {
				...
				System.out.println("No existen pasos con resultados asociados al id de ejecuci�n: " + idEjecucion);
			}
